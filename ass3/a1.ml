open A0
(* code goes here *)
type answer = Num of bigint | Bool of bool | Tup of int * (answer list) | Var of string;;
...
let rec eval t =
  ...
  match t with
    ...
    | Nega a -> Num (minus (nunwrap (eval a) ))
...
| PAREN :: ols -> raise InvalidArgument
...
| (TUPLE a) :: ols -> (let tmp = extractlist a [] bl in stackmc (( Tup (a , (get 1 tmp)  ) ) :: (get 2 tmp)) ols)
| PROJ which -> (match bl with Tup(a,alist)::bls -> stackmc ((get which alist)::bls) ols | _ -> raise InvalidArgument )
| _ -> raise InvalidArgument;;
...
let rec compile t =
  ...
  | Tuple (a,explist) -> (map compile explist) @ [ TUPLE a ]
